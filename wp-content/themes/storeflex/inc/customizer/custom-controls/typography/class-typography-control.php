...
if ( ! class_exists( 'StoreFlex_Control_Typography' ) ) {
     ...
     * @since 1.0.0
     */
    class StoreFlex_Control_Typography extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->l10n = wp_parse_args(
                $this->l10n,
                array(
                    'family'        => esc_html__( 'Font Family', 'storeflex' ),
                    'weight'        => esc_html__( 'Font Weight', 'storeflex' ),
                    'style'         => esc_html__( 'Font Style', 'storeflex' ),
                    'transform'     => esc_html__( 'Font Transform', 'storeflex' ),
                    'decoration'    => esc_html__( 'Font Decoration', 'storeflex' )
                )
            );
        }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script( 'storeflex-typo-ajax-script', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/typo-ajax.js', array( 'jquery', 'select2' ), false, true );
            
            wp_localize_script( 'storeflex-typo-ajax-script', 'ajax_script', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
        }
        ...
        function get_font_families() {
            ...
            $storeflex_google_font = get_option( 'storeflex_google_font' );

            foreach ( $storeflex_google_font as $key => $value ) {
        ...
        public function get_font_weight_choices() {
            ...
            if ( $this->settings['family']->id ) {
                ...
                $get_font_family = storeflex_get_customizer_option_value( $font_family_id );
                $variants_array = $storeflex_font_list[$get_font_family]['0'];

                if ( is_array( $variants_array ) ) {
                    $options_array = array(
                        'inherit' => __( 'Inherit', 'storeflex' )
                    );
                    foreach ( $variants_array  as $variants ) {
                        $options_array[$variants] = storeflex_convert_font_variants( $variants );
                    }

                    return $options_array;
                } else {
                    return array(
                        'inherit'   => esc_html__( 'Inherit', 'storeflex' ),
                        '400'       => esc_html__( 'Normal 400', 'storeflex' ),
                        '700'       => esc_html__( 'Bold 700', 'storeflex' ),
                    );
                }
            } else {
                return array(
        ...
        public function get_font_style_choices() {
            ...
            $font_style_choices = apply_filters( 'storeflex_font_style_choices',
                array(
                    'inherit'   => __( 'Inherit', 'storeflex' ),
                    'normal'    => __( 'Normal', 'storeflex' ),
                    'italic'    => __( 'Italic', 'storeflex' ),
                )
            );
        ...
        public function get_font_transform_choices() {
            ...
            $font_transform_choices = apply_filters( 'storeflex_font_transform_choices',
                array(
                    'inherit'       => __( 'Inherit', 'storeflex' ),
                    'lowercase'     => __( 'Lowercase', 'storeflex' ),
                    'capitalize'    => __( 'Capitalize', 'storeflex' ),
                    'uppercase'     => __( 'Uppercase', 'storeflex' ),
                )
            );
        ...
        public function get_font_decoration_choices() {
            ...
            $font_decoration_choices = apply_filters( 'storeflex_font_decoration_choices',
                array(
                    'inherit'       => __( 'Inherit', 'storeflex' ),
                    'underline'     => __( 'Underline', 'storeflex' ),
                    'line-through'  => __( 'Line-through', 'storeflex' ),
                )
            );
